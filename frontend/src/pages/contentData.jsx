...
import React, { useState, useContext } from "react";
// Assuming NavInsideBar is in the correct path
import NavInsideBar from "../components/NavInsideBar";
// Assuming AppContent is correctly defined and exported from its context file
import { AppContent } from "../context/AppContext";
import axios from "axios";
import { toast } from "react-toastify";
import 'react-toastify/dist/ReactToastify.css'; // Ensure toast styles are imported

/* Reusable Input Component (Corrected for Name capitalization) */
const Input = ({
  label,
  name,
  type = "text",
  ...
  <div>
    ...
    <input
      className={`w-full rounded-lg border border-slate-300 px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none ${
        name === "name" ? "capitalize" : "" // Apply 'capitalize' class only to the 'name' input
      }`}
      name={name}
      type={type}
      value={value}
      ...
      min={min}
      max={max}
      // CSS for automatic capitalization is added via className.
      // Make sure your Tailwind CSS setup includes 'capitalize' utility.
    />
  </div>
);
...
const ContentData = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    religion: "",
    nationality: "",
    category: "", // Correctly initialized
    dateOfAdmission: "",
    dateOfLeaving: "",
    aadhaar: "",
    contactNo: "",
  ...
  const handleChange = (e) => {
    ...
    // Convert 'name' value to title case for storage, if it's the 'name' field
    const finalValue = name === 'name' ? value.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ') : value;

    setFormData((prev) => ({
      ...prev,
      [name]: files ? files[0] : finalValue, // Use finalValue for text inputs
    }));
  };

  const handleNext = () => {
    // Current validation logic for mandatory fields on step 1
    if (
      step === 1 &&
      (!formData.name ||
        !formData.dob ||
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Simplified submission logic for brevity, assuming backend is fine
    try {
      const finalData = new FormData();
      Object.entries(formData).forEach(([key, value]) => {
        finalData.append(key, value);
      ...
      if (data.success) {
        toast.success(data.message);
        // Reset form data by creating a new object with empty strings/nulls
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-slate-300">
      ...
      <div className="flex flex-col items-center pt-6 px-4">
        ...
        >
          ...
          {/* Step 5 - Course & Address (Category fix is here) */}
          {step === 5 && (
            <div className="grid grid-cols-1 gap-6">
              <Input
                label="Address"
              ...
              {/* CORRECTED: Replaced inline select with the reusable Select component for Category */}
              <Select
                label="Category"
                name="category"
                value={formData.category}
                onChange={handleChange}
                required={true} // Made required to maintain previous logic
